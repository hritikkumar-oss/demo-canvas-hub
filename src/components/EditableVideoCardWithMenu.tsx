...
const EditableVideoCardWithMenu = (props: EditableVideoCardWithMenuProps) => {
  ...
  const handleSave = (data: EditFormData) => {
    if (props.productId) {
      updateVideo(props.productId, props.id, {
        title: data.title,
    ...
    <div className="relative group">
      ...
      {props.isAdmin && (
        <Button
          variant="ghost"
          size="sm"
          className="absolute top-2 right-12 h-8 w-8 p-0 bg-white/90 backdrop-blur-sm hover:bg-white/95 opacity-0 group-hover:opacity-100 transition-opacity z-10"
          onClick={handleEdit}
        >
      ...
      <EditCardModal
        ...
        initialData={{
          ...
          redirectUrl: `/video/${props.productId}/${props.id}`
        }}
        type="video"
      />
    </div>
